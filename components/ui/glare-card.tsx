...
export const GlareCard = ({
  children,
  className,
  radius = 18, // ✅ default reduced (was 48px)
}: {
  children: React.ReactNode;
  className?: string;
  radius?: number; // ✅ allow per-card control
}) => {
  ...
  return (
    <div
      ...
      className={cn(
        // ✅ removed fixed width/aspect ratio so your grid controls sizing
        "relative isolate contain-[layout_style] perspective-[600px] will-change-transform",
        "transition-transform duration-(--duration) ease-(--easing) delay-(--delay)"
      )}
      onPointerMove={(event) => {
    ...
    >
      <div
        className={cn(
          "h-full w-full grid overflow-hidden border border-slate-800",
          "origin-center will-change-transform",
          "transition-transform duration-(--duration) ease-(--easing) delay-(--delay)",
          "transform-[rotateY(var(--r-x))_rotateX(var(--r-y))]",
          "rounded-lg",
          "hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none"
        )}
      >
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          {/* ✅ IMPORTANT: allow caller to set radius via their own wrapper, but this keeps internal clipped correctly */}
